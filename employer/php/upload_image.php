...
<?php
	if (isset($_POST['upload-img'])) {
		...
		$img_upload_dir = "../../employer_img/";
		$img_info = $_FILES["image"]["name"];
		$file_extension = strtolower(pathinfo($img_info, PATHINFO_EXTENSION));

		$target_file = $img_upload_dir . basename("".$sess_comp_member_no.".".$file_extension."");

		$image = "".$sess_comp_member_no.".".$file_extension."";
		...
		if ($_FILES["image"]["size"] > 500000) {
			...
			if( $file_extension != "jpg" && $file_extension != "png" && $file_extension != "jpeg" ) {
				...
				if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
					...
					$update_img = "UPDATE tbl_users SET image = '$image' WHERE member_no = '$sess_comp_member_no'";
					$update_img_con = mysqli_query($connection, $update_img);

					if ($update_img_con) {
						$get_image_details = "SELECT image FROM tbl_users WHERE member_no = '$sess_comp_member_no'";
						$run_image_details = mysqli_query($connection, $get_image_details);

						while ($row_image_details = mysqli_fetch_array($run_image_details)){

							$_SESSION['comp_image'] = $row_image_details['image'];
